...
public class ReplyController {
    ...
    public List<ReplyDTO> getList(@PathVariable Long bno) { // 얘는 REst라서 RequestParam을 쓸 수 없다.
        log.info("{} 댓글 요청 ", bno);
        return replyService.getList(bno);
    }

    @GetMapping("/{rno}")
    public ReplyDTO getRow(@PathVariable Long rno) { // 얘는 REst라서 RequestParam을 쓸 수 없다.
        log.info("{} 댓글 요청 ", rno);
